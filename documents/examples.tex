...
$\mathit{function}$. Furthermore the real numbers $\mathbb{R}$ has a shortcut
$\backslash$R and the same goes for \C, \Q, \F and more. All these macros ensure
to go into math mode, so you will not get any compile errors in plain writing.
Finally with \emph{mathcal} you can get caligraphic writing.
...
the \NP-complete problem \SAT\ multiple times, you may want to define \SAT\ as a
local macro, since it is not part of the preamble.
...
$\Det{A}$, the equivalence class $\eqclass{v}{\V}$, the matrix representation
$\matrep{\V}{L}{\W}$, the $\sgn{\sigma}$ and the $\Span{\seq{v}{1}{n}}$ can be
made easily with the macros coded. The vectors \allones\ and \allzeros\ also
have a shortcut.
